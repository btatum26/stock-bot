...
`CLI.py` is the LLM-friendly command-line interface for the research engine.  
Run all commands from the **repo root** with `uv run python CLI.py <command>`.
...
```bash
uv run python CLI.py features --json
```
...
uv run python CLI.py edit my_strategy --add-feature MovingAverage --feature-params period=50 type=EMA

# Add a second moving average (same feature ID is fine — two entries are created)
uv run python CLI.py edit my_strategy --add-feature MovingAverage --feature-params period=200 type=SMA

# Set hyperparameters
uv run python CLI.py edit my_strategy --set-hparam oversold=30.0 overbought=70.0 stop_loss=0.05

# Set optimisation bounds for a param
uv run python CLI.py edit my_strategy --set-bound oversold=20.0,40.0 overbought=60.0,80.0

# Remove a feature by ID (removes all entries with that ID)
uv run python CLI.py edit my_strategy --remove-feature MovingAverage

# Wipe all features and start over
uv run python CLI.py edit my_strategy --clear-features
```
...
```bash
uv run python CLI.py inspect my_strategy
```
...
```bash
uv run python CLI.py show-model my_strategy
```
...
```bash
uv run python CLI.py backtest my_strategy --tickers AAPL --interval 1d --start 2020-01-01 --end 2024-01-01

# or with a named universe
uv run python CLI.py backtest my_strategy --universe MEGA_CAP_TECH --interval 1d
```
...
```bash
uv run python CLI.py backtest my_strategy --tickers AAPL --debug
```

### 10. Train (hyperparameter optimisation)

```bash
uv run python CLI.py train my_strategy --tickers AAPL --interval 1d --start 2018-01-01 --end 2024-01-01

# multi-ticker ML training with a universe
uv run python CLI.py train my_strategy --universe DOW_30 --interval 1d
```
...
```bash
uv run python CLI.py list [--json]
```
...
```bash
uv run python CLI.py inspect <strategy> [--json]
```
...
```bash
uv run python CLI.py init <strategy>
```
...
```bash
uv run python CLI.py edit <strategy> [options]
```
...
```bash
uv run python CLI.py show-context <strategy>
```
...
```bash
uv run python CLI.py show-model <strategy>
```
...
```bash
uv run python CLI.py data-info [--ticker AAPL] [--interval 1d]
```
...
```bash
uv run python CLI.py validate <strategy>
```
...
```bash
uv run python CLI.py backtest <strategy> \
    --tickers AAPL,MSFT \
    --interval 1d \
    --start 2020-01-01 \
    --end 2024-01-01 \
...
uv run python CLI.py backtest <strategy> --universe MEGA_CAP_TECH --interval 1d
```
...
```bash
uv run python CLI.py portfolio <strategy> \
...
# or use a named universe
uv run python CLI.py portfolio <strategy> --universe SECTOR_ETFS --interval 1d
```
...
```bash
uv run python CLI.py train <strategy> \
    --tickers AAPL \
    --interval 1d \
    --start 2018-01-01 \
    --end 2024-01-01 \
...
uv run python CLI.py train <strategy> --universe DOW_30 --interval 1d
```
...
```bash
uv run python CLI.py signal <strategy> --tickers AAPL,MSFT

# or use a named universe
uv run python CLI.py signal <strategy> --universe MEGA_CAP_TECH
```
...
uv run python CLI.py trial-counts

# Manually backfill counts for a strategy
uv run python CLI.py trial-counts --set my_strategy --backtest-count 12 --train-count 3
```
...
```bash
uv run python CLI.py sensitivity <strategy> \
...
# or use a named universe
uv run python CLI.py sensitivity <strategy> --universe MEGA_CAP_TECH --interval 1d
```
...
```bash
uv run python CLI.py signal-stability <strategy> \
    --tickers AAPL,MSFT,NVDA \
    --interval 1d

# or use a named universe
uv run python CLI.py signal-stability <strategy> --universe MEGA_CAP_TECH
```
...
```bash
uv run python CLI.py diagnose <strategy> \
    --tickers AAPL,MSFT,NVDA \
    --interval 1d \
    [--start 2019-01-01] \
    [--sensitivity]
...
# or use a named universe
uv run python CLI.py diagnose <strategy> --universe MEGA_CAP_TECH --interval 1d
```
...
uv run python CLI.py ic <strategy> --universe DOW_30 --interval 1d --start 2015-01-01
```
...
uv run python CLI.py backtest my_strategy --tickers AAPL,MSFT,NVDA --interval 1d
